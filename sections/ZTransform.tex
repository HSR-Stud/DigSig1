...
\begin{tabularx}{0.6\textwidth}{|l|X|}
	...
	\hline
	$e^{-i a n}$ &	$X(\omega) = 2\pi\cdot \delta (\omega +a),     -\pi \leq a < \pi$ \\
	\hline
	$\cos(a n) $ & $X(\omega) = \pi [\delta (\omega +a)+\delta (\omega -a)],     -\pi < a < \pi $\\ &
	$X_{2\pi}(\omega) = \pi \sum_{k=-\infty}^{\infty} \left[ \delta (\omega - a - 2\pi k) + \delta (\omega + a + 2\pi k) \right] $ \\
	\hline
	$\sin(a n) $ &	$X_{2\pi}(\omega) = \frac{\pi}{i} \sum_{k=-\infty}^{\infty} \left[ \delta (\omega - a - 2\pi k) - \delta ( \omega + a + 2\pi k) \right] $  	\\
	\hline
\end{tabularx} \\ \\
